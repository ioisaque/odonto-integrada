...
<?php
	...
	public function setup_notice(){
		if( strpos( get_current_screen()->id, 'settings_page_disable_comments_settings' ) === 0 )
			return;
		$hascaps = $this->networkactive ? is_network_admin() && current_user_can( 'manage_network_plugins' ) : current_user_can( 'manage_comments' );
		if( $hascaps ) {
	...
	*/
	public function plugin_actions_links( $links, $file ) {
		static $plugin;
		$plugin = plugin_basename( __FILE__ );
		if( $file == $plugin && current_user_can('manage_comments') ) {
			array_unshift(
				$links,
	...
	public function settings_menu() {
		...
		if( $this->networkactive )
			...
			add_submenu_page( 'options-general.php', $title, $title, 'manage_comments', 'disable_comments_settings', array( $this, 'settings_page' ) );
	}
	...
	public function tools_menu() {
		...
		if( $this->networkactive )
			...
			add_submenu_page( 'tools.php', $title, $title, 'manage_comments', 'disable_comments_tools', array( $this, 'tools_page' ) );
	}
