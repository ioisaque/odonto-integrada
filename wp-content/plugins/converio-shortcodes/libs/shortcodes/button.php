...
//	$arr_color = array('orange', 'green', 'turquoise', 'blue', 'purple', 'pink', 'red', 'dark-gray', 'light-gray');
	...
	$output = '<a href="'.$link.'" class="btn'.(in_array($size, $arr_size) ? ' '.$size : '').($light == "yes" ? ' light' : '').($color != '' ? ' '.$color : '').($class != '' ? ' '.$class : '').'"'.($open_in_new_window == "yes" ? ' target="_blank"' : '').($rel != '' ? ' rel="'.$rel.'"' : '').($title != '' ? ' title="'.$title.'"' : '').'>';
	if (!empty($icon))
