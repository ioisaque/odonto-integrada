	...
	<article class="main single contact">
		<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
			...
			<form action="<?php the_permalink() ?>" method="post" class="contact-form">
				...
				?>
				<p class="half">
					<label for="sender_name"><?php esc_attr_e('Nome','converio');?></label><input name="sender_name" id="sender_name" value="<?php echo esc_attr($_POST['sender_name']);?>" <?php if(in_array('sender_name', $response['errorFields'])) echo 'class="error"' ?>>
				</p>
				<p class="half">
				...
				<p>
					<label for="topic"><?php esc_attr_e('Assunto','converio');?></label><select name="subject" id="subject" <?php if(in_array('subject', $response['errorFields'])) echo 'class="error"' ?>>
				...
				<p>
					<label for="message"><?php esc_attr_e('Mensagem','converio');?></label><textarea name="message" id="message" rows="5" cols="20" <?php if(in_array('message', $response['errorFields'])) echo 'class="error"' ?>><?php echo esc_attr($_POST['message']); ?></textarea>
				</p>
				<p><button name="send" type="submit" value="1"><?php esc_attr_e('Enviar mensagem','converio');?></button></p>
			</form>
