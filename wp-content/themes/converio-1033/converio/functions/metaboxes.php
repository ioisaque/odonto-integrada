...
function converio_draw_display_metabox($post) {
	...
	$hide_featured_image = isset($data['hide_featured_image']) ? esc_attr($data['hide_featured_image'][0]) : 0;
	...
	?>
	<p><label for="hide_title"><?php esc_attr_e('Show title?', 'converio'); ?></label> <select name="hide_title" id="hide_title">
	...
	<p><label for="hide_featured_image"><?php _e('Show featured image?', 'converio'); ?></label> <select name="hide_featured_image" id="hide_featured_image">
		<option value="0" <?php if($hide_featured_image == 0) echo 'selected="selected"'; ?>>As set in Theme Customizer (default)</option>
		<option value="1" <?php if($hide_featured_image == 1) echo 'selected="selected"'; ?>>Show</option>
		<option value="2" <?php if($hide_featured_image == 2) echo 'selected="selected"'; ?>>Don't show</option>
	</select></p>
...
function converio_save_display_metabox($page_id) {
	...
	if(isset($_POST['hide_featured_image'])) {
		update_post_meta($page_id, 'hide_featured_image', strip_tags($_POST['hide_featured_image']));
	}
