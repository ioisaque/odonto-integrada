...
function converio_customize_register_blog($wp_customize) {
	...
	//title in breadcrumb on single page
	$wp_customize->add_setting('breadcrumb_header_single', array(
		'default' => 2,
		'sanitize_callback' => ''
	) );
	$wp_customize->add_control('breadcrumb_header_single', array(
		'label' => esc_attr__('What text to show in main breadcrumb as title', 'converio'),
		'section' => 'blog',
		'settings' => 'breadcrumb_header_single', 
		'type' => 'radio',
		'choices' => array(
			1 => esc_attr__('Title of post', 'converio'),
			2 => esc_attr__('Blog', 'converio')
		)
	));
	...
	$wp_customize->add_control(new WP_Customize_Control($wp_customize, 'hide_featured_image',
        array(
            'label'     => esc_attr__('Hide featured images in posts and pages', 'converio'),
            'section'   => 'blog',
