<?php
//necessary to use is_plugin_active function
...
if ( is_plugin_active( 'one-click-demo-import/one-click-demo-import.php' ) ) {
	//function for importing Revolution Slider (including files using require_once)
	function converio_import_revolution_slider() {
		if ( class_exists( 'RevSlider' ) ) {
			$slider_array = array(
			   get_template_directory()."/dummy-data/slider1.zip"
			 );
	...
	include(get_template_directory().'/functions/one-click-demo-import.php');
}
...
function converio_widgets() {
	...
	if (class_exists('Woo_Sidebars')) {	
		$sidebars = get_posts( array( 
			'post_type'        => 'sidebar', 
			'posts_per_page'   => intval( apply_filters( 'woosidebars_upper_limit', 200 ) ), 
...
if (class_exists('Woo_Sidebars')) {
	function converio_remove_woo_sidebars() {
		global $woosidebars;
		remove_action( 'widgets_init', array( $woosidebars, 'register_custom_sidebars' ) );
	}
	add_action( 'init', 'converio_remove_woo_sidebars' );
}
...
function converio_scripts() {
	...
	if (isset($maps_api_key) && !empty($maps_api_key)) {
		$maps_script .= '?key='.converio_sanitize_text_decode($maps_api_key);
	}
	
    wp_register_script('googlemaps', $maps_script, array(), 1, true);
